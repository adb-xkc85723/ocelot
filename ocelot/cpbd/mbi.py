...
class MBI(PhysProc):
    ...
    def __init__(self, lattice: MagneticLattice, step: float=1, lamb_range: list=(1e-6, 50e-6, 10), slices: int=0, lsc: bool=True, csr: bool=True):
        ...
        self.slice_params = []
        if self.slice > 0:
            self.bf = [[[] for i in range(len(self.lamb_range))] for j in range(self.slice)]
            self.pf = [[[] for i in range(len(self.lamb_range))] for j in range(self.slice)]
    ...
    def get_slice_params(self, p_array: ParticleArray, slices: bool=False) -> dict:
        ...
        for k, v in slice_params.__dict__.items():
            if k in ['beta_x', 'alpha_x', 'beta_y', 'alpha_y', 'I', 'sig_x', 'sig_y']:
                ...
                sli.update({k: np.mean(v[sli_min: sli_max])*1e-6 / (np.mean(slice_params.me[sli_min: sli_max] / 1e6 / m_e_MeV))})
    ...
    def apply(self, p_array: ParticleArray, dz: float) -> None:
        ...
        if self.slice > 0:
            ...
            for i in range(len(t_his) - 1):
                inds = np.where(np.logical_and(p_array_c.tau()>=t_his[i], p_array_c.tau()<=t_his[i+1]))[0]
                ...
                res = lattice_transfer_map_z(self.lattice, self.slice_params[i][0]['me'], z0)
                ltm.append(res[0])
                elem = res[1]
            self.optics_map.append(self.ltm[-1])
        else:
            self.slice_params.append(self.get_slice_params(p_array_c))
            z0 = self.slice_params[-1]['s'] - self.slice_params[0]['s']
            ltm, elem = lattice_transfer_map_z(self.lattice, self.slice_params[0]['me'], z0)
            self.optics_map.append(ltm)
        self.dist.append(self.z0)
        if self.slice > 0:
            p_array.bf, p_array.pf = self.get_bf_slice(self.lamb_range, self.slice_params, self.optics_map, self.slice, elem)
        else:
            p_array.bf, p_array.pf = self.get_bf(self.lamb_range, self.slice_params, self.optics_map, elem)


    def get_bf(self, lamb_range: list, slice_params: list, optics_map: list, elem: Element) -> list:
        """
    ...
    def get_bf_slice(self, lamb_range: list, slice_params: list, optics_map: list, slices: int, elem: Element) -> list:
        """
    ...
    def ldtaus(self, lamb: list, slice_params: list, optics_map: list, i1: int) -> float:
        '''
    ...
    def kernel_K1(self, lamb: float, slice_params: list, optics_map: list, i1: int, elem: Element) -> float:
        '''
        Kernel K1 (Eq. (A17b))
    ...
    def kernel_K2(self, lamb: float, slice_params: list, optics_map: list, i1: int, elem: Element) -> float:
        '''
        Kernel K2 (Eq. (A17c))
